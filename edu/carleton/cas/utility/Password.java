...
public class Password {
   private static final char[] disallowedChars = new char[]{'I', 'l', 'O', '0'};
   ...
   public static void main(String[] args) throws NoSuchAlgorithmException {
      ...
      System.out.println(ucPassword + " " + ucPassword.length());
   ...
   public static boolean isValid(String passcode) {
      return isValid(passcode, disallowedChars);
   }

   public static boolean isValid(String passcode, char[] invalidChars) {
      if (passcode.length() != defaultLength) {
         return false;
      } else {
         char[] chars = passcode.toCharArray();

         for(int i = 0; i < chars.length; ++i) {
            if (chars[i] > '9' && (chars[i] < 'a' || chars[i] > 'z') && (chars[i] < 'A' || chars[i] > 'Z')) {
               return false;
            }
   ...
   public static String getPassCode() {
      try {
         return getPassCode(defaultLength);
   ...
   public static String getPassCode(int length) throws NoSuchAlgorithmException {
      return getPassCode(length, disallowedChars);
   }

   public static String getPassCode(int length, char[] disallowed) throws NoSuchAlgorithmException {
      if (length < 0) {
         return "";
      } else {
         if (length > 32) {
