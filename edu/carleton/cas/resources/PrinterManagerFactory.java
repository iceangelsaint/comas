...
public class PrinterManagerFactory {
   // $FF: synthetic field
   private static volatile int[] $SWITCH_TABLE$edu$carleton$cas$constants$ClientShared$OS;

   public static PrinterManagerInterface create(SessionConfigurationModeMonitor scnm) {
      ClientShared.OS os = ClientShared.getOS();
      switch (os) {
         case windows:
            return new PrinterManagerForWindows(scnm);
         case macOS:
            return new PrinterManagerForMacOS(scnm);
         case linux:
            return new PrinterManagerForLinux(scnm);
         default:
            return null;
      }
   }
